...
public class Dashboard extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String maintenancesql="SELECT COUNT(*) FROM maintenance";
            ...
            int damagedCount = jdbcTemplate.queryForObject(damagedCountSql, Integer.class);
              ...
              int activUsersCount = jdbcTemplate.queryForObject(activeUserssql, Integer.class);
              int maintenanceCount=jdbcTemplate.queryForObject(maintenancesql, Integer.class);

            // Set attributes to be passed to the JSP
            request.setAttribute("activUsersCount", activUsersCount);
            ...
            RequestDispatcher dispatcher = request.getRequestDispatcher((String)session.getAttribute("level")+"_dashboard.htm");
            dispatcher.forward(request, response);
        } catch (Exception e) {
            e.printStackTrace();
            response.sendRedirect(session.getAttribute("level")+"_dashboard.htm");
        }
    }
